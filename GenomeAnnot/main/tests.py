...
class ViewTest(TestCase):
    ...
    def test_main_annotate_view_base(self):
        ...
        response = self.client.get(
            url, follow=True
        )  # follow to go to each redirection
    ...
    def test_main_explore_view(self):
        ...
        url = reverse("main:exploreGenome")
        response = client.get(url, follow=True)
        # Check 200 status code
        self.assertEqual(response.status_code, 200)
        # Check template
        self.assertTemplateUsed(response, "main/explore/main_exploreGenome.html")
        # Check the value of active_tab
        self.assertEqual(response.context["active_tab"], "explore")
        self.assertEqual(response.context["active_subtab"], "genome")

        # get request to the explore view
        url = reverse("main:exploreGenePep")
        response = client.get(url, follow=True)
        # Check 200 status code
        self.assertEqual(response.status_code, 200)
        # Check template
        self.assertTemplateUsed(response, "main/explore/main_exploreGenePep.html")
        # Check the value of active_tab
        self.assertEqual(response.context["active_tab"], "explore")
        self.assertEqual(response.context["active_subtab"], "gene")
...
class BlastTest(TestCase):
    ...
    def test_blast_view(self, mock_read, mock_qblast):
        ...
        response = self.client.post(
            url, {"sequence": "ATGC", "program": "blastn", "alignments": 5}
        )
    ...
    def test_blast_view_error(self):
        ...
        response = self.client.post(
            url,
            {"sequence": "invalid_sequence", "program": "blastn", "alignments": 5},
            follow=True,
        )
